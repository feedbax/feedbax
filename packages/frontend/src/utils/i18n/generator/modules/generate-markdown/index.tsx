...
import config from '@/utils/i18n/config.json';
import getHyphen from '@/utils/i18n/generator/helper/get-hypen';

import type { Context } from '@/utils/i18n/generator';
  ...
  async function generateMarkdown({ translationDir }: Context): Promise<void> {
    ...
    for (let i = 0; i < config.locales.length; i += 1) {
      ...
      for (let j = 0; j < fileNames.length; j += 1) {
        ...
        loaders.push({ locale, componentPath: `@/utils/i18n/locales/${locale}/__generated/${fileParsedPath.name}` });
